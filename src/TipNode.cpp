...
void TipNode::computeUpred(const maternVec & covParasSp, const double & scaling, const spatialcoor & predictLocations,
  ...
  if (m_predsInNode.size() > 0) {
    ...
    m_UmatList.at(0) = computeCovMat(subLocations, brickList.at(0)->GetKnotsCoor(), covParasSp, scaling, spaceNuggetSD, distMethod) ;

    for (uint l = 1; l <= m_depth; l++) {
      mat firstTerm = computeCovMat(subLocations, brickList.at(l)->GetKnotsCoor(), covParasSp, scaling, spaceNuggetSD, distMethod) ;
